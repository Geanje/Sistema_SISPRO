...
require_once "../modelos/proforma1.0.php";
...
switch ($_GET["op"]){
	case 'guardaryeditar':
		if (empty($idproforma)){
			$rspta=$proforma->insertar($idusuario,$idcliente,$correlativo,$referencia,$fecha_hora,$validez,$tiempoEntrega,$_POST["idarticulo"],$_POST["cantidad"],$_POST["precio_venta"],$_POST["serieProforma"],$total_venta,$igv_total,$igv_asig);
			echo $rspta ?  "No se pudieron registrar todos los datos de la Proforma": "¡Ha registrado la Proforma exitosamente";
		}
		else {
		}
	break;
	...
	case 'listar':
		$rspta=$proforma->listar();
 		...
 		while ($reg=$rspta->fetch_object()){
 			...
 			// }else{
 				$url='../reportes/proformas1.0.php?id=';
