...
function init(){
	...
	$.post("../ajax/proforma1.0.php?op=selectCliente", function(r){
	            $("#idcliente").html(r);
	...
	$.post("../ajax/proforma1.0.php?op=selectMoneda",function(r){
			$('#moneda').html(r);
			$('#moneda').selectpicker('refresh');

	});

	$.post("../ajax/proforma1.0.php?op=selecTipoIGV",function(r){
		$('#impuesto').html(r);
...
function listar()//al parecer lista las proformas guardadas
{
	...
	tabla=$('#tbllistado').dataTable(
	{
		...
		"ajax":
				{
					url: '../ajax/proforma1.0.php?op=listar',
					type : "get",
					dataType : "json",
					error: function(e){
...
function listarArticulos()
{
	tabla=$('#tblarticulos').dataTable(
	{
		...
		"ajax":
				{
					url: '../ajax/proforma1.0.php?op=listarArticulosVenta',
					type : "get",
					dataType : "json",
					error: function(e){
						console.log(e.responseText);
...
function guardaryeditar(e)
{
	...
	$.ajax({
		url: "../ajax/proforma1.0.php?op=guardaryeditar",
	    type: "POST",
	    data: formData,
	    contentType: false,
	    processData: false,
...
{
	$.post("../ajax/proforma1.0.php?op=mostrar",{idproforma : idproforma}, function(data, status)
	{
		data = JSON.parse(data);
		mostrarform(true);
 	...
 	$.post("../ajax/proforma1.0.php?op=listarDetalle&id="+idproforma,function(r){
	        $("#detalles").html(r);
	});
}
...
function anular(idproforma)
{
	bootbox.confirm("¿Está Seguro de anular la venta?", function(result){
		if(result)
        {
        	$.post("../ajax/proforma1.0.php?op=anular", {idproforma : idproforma}, function(e){
        		bootbox.alert(e);
	            tabla.ajax.reload();
        	});
        }
...
function agregarDetalle(idarticulo,articulo,unidad_medida,precio_venta,afectacion)
  {
    ...
    if (idarticulo!="")
    {
    	...
    	'<td><input type="text" name="serieProforma[]" id="serieProforma[]" ></td>'+
